...
- **Room avatars (pixel art)**: the bot automatically sets a bundled pixel-art avatar on every room it manages — the space and each project room pick from a 12-image pool by hashing the instance/project name (same name → same image forever, different ones usually differ), and the management room gets a dedicated image. Existing rooms are branded on the next start too; an avatar you set manually is never replaced. Not a fan of the art? Replace any PNG in `assets/avatars/` (same filename) with your own.
