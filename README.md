...
- **Room avatars (candy art)**: the bot automatically sets a bundled high-res candy-style avatar (fluffy marshmallow animals on a cotton-candy background, 512×512) on every room it manages — the space and each project room pick from a 12-image pool by hashing the instance/project name (same name → same image forever, different ones usually differ), and the management room gets a dedicated image. Existing rooms are branded on the next start too; an avatar you set manually is never replaced. When the bundled pool ships a full restyle, rooms the bot itself branded get the new art automatically on the next start (your own avatars are still never touched). Not a fan of the art? Replace any PNG in `assets/avatars/` (same filename) with your own.
