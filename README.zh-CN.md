...
- **头像(三套素材)**:bot 自动为它托管的一切配上内置 512×512 头像——而且每种实体一套,一眼就能分辨:**空间**用冷色调**风景**套(极光、海岛、森林…),**项目房间**用暖色**小屋**套(红顶小屋、蘑菇屋、灯塔…),**管理房间**用小屋套专属的金顶城堡,**bot 账号自己**(agent 的脸,显示在成员列表和它发的每条消息旁)用**动物**套。同一套内按实例名/项目名哈希取图(同名永远同一张,不同部署基本不同)。存量房间下次启动自动补齐,手动设置过的头像绝不被覆盖——除非某套素材整体换风格,那一次该套管辖的房间(和 bot 头像)会在下次启动统一刷成新图,且各套独立迁移、互不牵连(此后你手动设置的头像恢复"绝不被覆盖")。不喜欢这些图?把 `assets/avatars/` 下的同名 PNG 换成你自己的即可,无需改代码。
